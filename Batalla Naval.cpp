...
int menu(){
	...
	cursor=load_bitmap("dis/cursor.bmp",NULL);     //imagen del cursor, una PAPA!
	...
	do {
		...
		if(key[KEY_A]){ // Si el usuario presiona A accederá al menu de ayuda.
			clear_bitmap(fondo); //Limpiamos la pantalla
			fondo_tab = load_bitmap("dis/pantalla.bmp",NULL); //Cargamos el fondo para ayuda.
			FILE *ayuda=fopen("utilidades/ayuda.txt", "rt"); //Abrimos el texto de ayuda
			if(ayuda==NULL){
				allegro_message("Error de archivos!");
				exit(1); //Si no se encuentra cerramos la ejecucion.
			}
			char frase[180]; //Vector de caracteres que soportara cada linea de texto.
			int y=30; //Coordenada Y que aumentara.
			fgets(frase, 150, ayuda);
			while(!feof(ayuda)){ //Ciclo de lectura del archivo.
				textprintf(fondo_tab,font,30,y,makecol(255, 255, 255),"%s", frase); //Impresion de ayuda.
				y+=20; //Movimiento de Y para imprimir debajo.
				fgets(frase, 180, ayuda);
			}
			draw_sprite(screen,fondo_tab,0,0); //imprime el fondo en pantalla
			while(!key[KEY_ESC]){
			}
			fclose(ayuda); //Si se presiona ESC se regresa al menu.
			clear(screen);
		}
		...
		textprintf(fondo,font,1000,30,makecol(0, 0, 0),"A = AYUDA"); 
